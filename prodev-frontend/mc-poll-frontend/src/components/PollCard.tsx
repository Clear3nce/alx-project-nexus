...
export default function PollCard({ poll }: PollCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="mb-4">
        <p className="text-gray-600 text-sm">Created: {new Date(poll.created_at).toLocaleDateString()}</p>
        <p className="text-gray-600 text-sm">Expires: {new Date(poll.expires_at).toLocaleDateString()}</p>
      ...
      <div className="space-y-2 mb-4">
        {poll.options.slice(0, 3).map((option) => (
          <div key={option.id} className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium text-blue-600">{option.vote_count} votes</span>
          </div>
        ))}
